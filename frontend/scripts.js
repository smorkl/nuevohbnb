...
// Iterate over the places data
  ...
  placeDiv.innerHTML = `
          ...
          <a class="details-button" href="place.html?place_id=${place.id}&image_number=${imageNumber}">View Details</a>
      `;
...
function displayPlaceDetails(place, imageNumber) {
  ...
  // Verificar que el contenedor existe antes de modificarlo
  if (!placeDetailsSection) {
      console.error("Error: No se encontró el contenedor 'place-details'.");
      return;
  }

  console.log("Datos del lugar:", place);
  console.log("Amenities recibidos:", place.amenities);

  // Limpiar el contenido actual del contenedor
  placeDetailsSection.innerHTML = "";
  placeDetailsSection.className = "place-card";

  console.log("Amenities data:", place.amenities);

  // Verificar si hay amenities y extraer sus nombres
  const amenitiesText = place.amenities && Array.isArray(place.amenities) 
      ? place.amenities.map(a => a.name || 'Unknown').join(', ') 
      : 'No amenities listed';

  // Asignar contenido HTML con los datos del lugar
  placeDetailsSection.innerHTML = `
      <img src="images/${imageNumber}.jpg" alt="Place Image" class="place-image">
      <h2>${place.title}</h2>
...
async function loadHeader(containerId, filePath) {
  try {
      ...
      console.error('Error en loadHeader:', error);
  }
}
